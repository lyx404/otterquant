  ...
  return (
    ...
    >
      ...
      {onboardingOpen && (
        ...
        <div className="space-y-2"><OnboardingTaskGroup title={tr("Create a factor", "创建 1 个因子")} progressLabel={tr("Task progress", "任务进度")} steps={[{ id: "connect-agent", label: tr("Connect an AI agent", "连接 AI 智能体") }, { id: "create-factor-with-agent", label: tr("Create a factor with an AI agent", "在 AI 智能体中创建 1 个因子") }]} completedStepStates={[false, factorCreationTaskComplete]} clickCompletedSteps onStepClick={(stepId) => { if (stepId === "connect-agent") { setOnboardingOpen(false); navigate("/account?tab=agent"); } if (stepId === "create-factor-with-agent") { setOnboardingOpen(false); setFactorGuideStep(0); setFactorGuideOpen(true); } }} /><OnboardingTaskGroup title={tr("Deploy a paper trading bot", "创建 1 个模拟交易")} progressLabel={tr("Task progress", "任务进度")} steps={[{ id: "create-strategy", label: tr("Create strategy", "创建策略") }, { id: "run-strategy", label: tr("Run strategy", "运行策略") }, { id: "deploy-paper", label: tr("Deploy strategy to paper trading", "将策略部署到模拟交易") }]} completedStepStates={[strategyCreationTaskComplete, strategyRunTaskComplete, paperDeploymentTaskComplete]} clickCompletedSteps onStepClick={(stepId) => { if (stepId === "create-strategy") { setOnboardingOpen(false); navigate("/strategies?onboarding=create-strategy"); } if (stepId === "run-strategy") { setOnboardingOpen(false); navigate("/strategies?onboarding=run-strategy"); } if (stepId === "deploy-paper") { setOnboardingOpen(false); navigate("/trade?onboarding=deploy-paper"); } }} /></div>
        </section>
      )}
