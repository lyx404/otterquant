/*
 * AppLayout — Amber/Orange + Warm Beige Design System
 * Light: #FDFCF6 warm beige / Dark: #020617 deep slate
 * Primary: Amber-500 #F59E0B / Amber-400 #FBBF24
 * Cards: #F5F1E1 beige (both modes) — paper-like
 * Nav active: rounded-full amber pill
 * Buttons: rounded-full | Cards: rounded-3xl | Inputs: rounded-xl
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Top Navigation Bar */}
      <header
        className={`sticky top-0 z-50 backdrop-blur-xl border-b border-border transition-transform duration-300 ease-in-out will-change-transform bg-[#FDFCF6]/80 dark:bg-slate-950/80 ${
      ...
      >
              ...
              <Link href="/account">
                <div className={`flex items-center gap-2 px-2.5 py-1 rounded-full border cursor-pointer transition-all duration-200 ease-in-out hover:scale-105 hover:shadow-sm ${
                  isActive("/account")
                    ...
                    : "bg-accent border-border hover:bg-amber-100 dark:hover:bg-slate-800 hover:border-primary/20"
                }`}>
        ...
        {/* Mobile Navigation */}
        {mobileMenuOpen && (
          <div className="md:hidden bg-[#FDFCF6]/95 dark:bg-slate-950/95 backdrop-blur-xl border-t border-border">
                ...
                return (
                  <Link key={item.path} href={item.path}>
                    <div
                      ...
                      className={`flex items-center gap-3 px-3 py-2.5 rounded-3xl text-sm font-medium transition-colors duration-200 ${
                        active
                          ? "bg-primary text-primary-foreground"
                          : "text-muted-foreground hover:text-foreground hover:bg-accent"
                      }`}
